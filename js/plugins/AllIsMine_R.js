...
// ----------------------------------------------------------------------------
// Version
// 1.0.3 2018/02/20 アイテム名だけ表示 ＆ OffSetX～OffSetYを変数指定で変更できるよう改造。英語版にはまだ微妙に非対応。
 ...
 * @min -1000
 * @max 1000
 * @desc This is a setting sets the variables of X position.
 * @default 0
 * 
 * @param VariablesY
 * @type number
 * @min -1000
 * @max 1000
 * @desc This is a setting sets the variables of Y position.
 * @default 0
 * 
 ...
 * @default 35
 * 
 * @param WaitTime_Variable
 * @type number
 * @desc This is a setting time to variable. The next item.60 = about 1 minute。
 * @default 300
 * 
 ...
 */
/*:ja
 * @plugindesc アイテム等を入手したときに吹き出しを表示させます（改造）
 * @author ツミオ
 *
 * 
 * 
 ...
 * @param 表示時間を制御する変数
 * @type number
 * @desc 次のアイテムを表示するまでの時間を設定する変数を指定します。 60 = 約1秒。
 * @default 300
 * 
 * @param ----表示する文字----
 * @desc 
...
(function() {
    ...
    param.variablesX      = getParamNumber(['VariablesX', 'X座標の変数']);
    param.variablesY      = getParamNumber(['VariablesY', 'Y座標の変数']);
...
//    param.offsetY         = getParamNumber(['OffSetY',            'Y座標のオフセット']);
    ...
    param.waitTime = getParamNumber(['WaitTime_Variable', '表示時間を制御する変数']);
    ...
    Window_AllIsMine.prototype.moveToPlayer = function () {
        //■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■        
        //変数格納先を指定
        //https://tm.lucky-duet.com/viewtopic.php?t=148
        var pos_x;
        var pos_y;
        pos_x = $gameVariables.value(param.variablesX) /*+ param.offsetX*/ - (this._width / 2);

        if (this.isPlayerUpper()) {
            pos_y = $gameVariables.value(param.variablesY) /*+ param.offsetY*/;
        } else {
            pos_y = $gameVariables.value(param.variablesY) /*+ param.offsetY*/ - (this._height * 1.5);
        }
...
//        this.y = pos_y;
        ...
        var pos_x;
        var pos_y;
        pos_x = $gameMap.events()[param.eventID]._realX + param.offsetX - (this._width / 2);

        if (this.isPlayerUpper()) {
            pos_y = $gameMap.events()[param.eventID]._realY + param.offsetY;
        } else {
            pos_y = $gameMap.events()[param.eventID]._realY + param.offsetY - (this._height * 1.5);
        }
        ...
        */
        ...
        var pos_x;
        var pos_y;
        pos_x  = param.offsetX - (this._width / 2);

        if(this.isPlayerUpper()){
            pos_y = param.offsetY;
        }else{
            pos_y = param.offsetY - (this._height * 1.5);
        }
    ...
    Window_AllIsMine.prototype.canNextQueue = function () {
        ...
        if (this._count_queue > $gameVariables.value(param.waitTime)) {
        //■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
            this._count_queue = 0;
            return true;
        }
...
//            SceneManager._scene._window_allMine._count_queue = param.waitTime;
            SceneManager._scene._window_allMine._count_queue = $gameVariables.value(param.waitTime);
            //■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
